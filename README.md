...
인증 연결 전까지 API의 현재 계정은 `DEV_ACCOUNT_ID`(기본 `acct-me`)로 잡는다. 개발 환경에서는 요청 헤더 `x-saegim-account-id`로 현재 계정만 임시 전환할 수 있다. 이후 Google OAuth 세션이 붙으면 이 경계를 실제 로그인 계정으로 교체한다.
