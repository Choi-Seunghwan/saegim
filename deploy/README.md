...
GitHub Actions는 `main` push에서만 자동으로 GHCR에 두 이미지를 푸시한다.
`dev` 검증 빌드는 비용을 아끼기 위해 자동 실행하지 않고, Actions 화면에서 `Build images` workflow를 수동 실행한다.
...
Mixpanel 같은 `NEXT_PUBLIC_*` 값도 웹 이미지 빌드 시점에 포함된다.
운영 값은 GitHub repository variables에 저장한다.
...
런타임 ConfigMap에는 아래 업로드 공개 설정을 둔다.
