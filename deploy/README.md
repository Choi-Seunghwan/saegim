...
- `saegim.chuz.dev` - 제품 대표/랜딩용으로 예약
- `web.saegim.chuz.dev` - Next.js 웹앱
- `api.saegim.chuz.dev` - NestJS API

초기 MVP는 `web.saegim.chuz.dev`와 `api.saegim.chuz.dev`를 먼저 연결한다.
...
```text
web.saegim.chuz.dev -> http://saegim-web.apps.svc.cluster.local:3000
api.saegim.chuz.dev -> http://saegim-api.apps.svc.cluster.local:4000
```

`saegim.chuz.dev`는 랜딩이 준비되면 `saegim-web` 또는 별도 landing 서비스로 연결한다.
