...
- `apps/web/src/components/SaegimShell.tsx`: 로그인 게이트(로그인/회원가입/Google/게스트)를 먼저 보여주고, 입장 상태는 임시로 `saegim_web_entry_state` localStorage에 저장한다. 실제 OAuth 세션 도입 시 교체한다.
