...
- `apps/api`: NestJS API. 현재 `/health`, `/auth/google`, `/auth/google/callback`, `/feed`, `/shelf`, `/posts/:postId`, `GET/PATCH /accounts/me`, `/accounts/recommended`, `POST /posts`, `GET/POST /posts/:postId/comments`, `POST/DELETE /accounts/:accountId/follow`, 좋아요/새김 토글로 기본 계약을 확인한다. 발행·좋아요·새김·구독·댓글은 PostgreSQL에 저장한다.
